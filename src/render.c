...
void render_paint(Editor *editor, HDC hdc, const RECT *update_rect, int tab_bar_offset) {
    ...
    if (GetFocus() == editor->hwnd) {
        ...
        int caret_x = (wrap_enabled ? (caret_display_col % editor->viewport.visible_cols) : caret_display_col - editor->viewport.scroll_x) * editor->viewport.char_width;
